...
const AdminProductForm = () => {
  ...
  useEffect(() => {
    if (isEditMode && currentProduct) {
      setFormData({
        ...
        images: currentProduct.images?.map(img => ({ id: img.id, url: img.imageUrl })) || [],
      });
    }
  ...
  const handleImageUpload = async () => {
    ...
    setIsUploading(true);
    try {
      const uploadPromises = imageFiles.map(file => uploadAPI.uploadImage(file, 'product'));
  ...
  return (
    <form onSubmit={handleSubmit} className="max-w-7xl mx-auto space-y-8 px-4 sm:px-6 lg:px-8 pb-16">
  ...
  <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="card bg-white shadow-sm border border-gray-200">
            <div className="card-body p-6 space-y-4">
              ...
              <div className="form-control">
                ...
                <textarea name="description" value={formData.description} onChange={handleChange} className="textarea textarea-bordered h-32"></textarea>
              </div>
            </div>
          </div>
          ...
          <div className="card bg-white shadow-sm border border-gray-200">
            <div className="card-body p-6 space-y-4">
              <h2 className="card-title text-lg font-semibold mb-4">Hình ảnh</h2>
              <div className="mt-2">
                <div 
                ...
                >
                  ...
                  <p className="mt-2 text-sm text-gray-600">
                    Kéo và thả ảnh vào đây, hoặc <span className="font-medium text-blue-600">nhấn để chọn tệp</span>
                  </p>
                  <input id="file-upload-input" type="file" multiple onChange={handleImageChange} className="hidden" />
                </div>
                {imageFiles.length > 0 && (
                  <div className="mt-4">
                    <p className="font-medium text-sm mb-2">Ảnh đã chọn:</p>
                    <ul className="list-disc list-inside text-sm text-gray-600">
                      {imageFiles.map((file, i) => <li key={i}>{file.name}</li>)}
                    </ul>
                    <button type="button" onClick={handleImageUpload} className="btn btn-secondary btn-sm mt-2" disabled={isUploading}>
              ...
              {formData.images.length > 0 && (
                <div className="mt-6">
                  <h3 className="text-md font-medium text-gray-800 mb-2">Ảnh hiện tại</h3>
                  <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-5">
                    {formData.images.map((image, index) => (
                      <div key={index} className="relative group aspect-square">
                        <img src={image.url} alt={`Product image ${index + 1}`} className="w-full h-full object-cover rounded-lg shadow" />
                        <button
                          type="button"
          ...
          <div className="card bg-white shadow-sm border border-gray-200">
            <div className="card-body p-6 space-y-4">
              <h2 className="card-title text-lg font-semibold mb-4">Giá & Tồn kho</h2>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div className="form-control">
                  <label className="label font-medium">Giá so sánh (tùy chọn)</label>
                  <input type="number" name="comparePrice" value={formData.comparePrice} onChange={handleChange} className="input input-bordered" min="0" />
                </div>
                <div className="form-control">
                  <label className="label font-medium">SKU (Mã sản phẩm)</label>
                  <input type="text" name="sku" value={formData.sku} onChange={handleChange} className="input input-bordered" required />
                </div>
                <div className="form-control">
                  <label className="label font-medium">Số lượng tồn kho</label>
                  <input type="number" name="quantity" value={formData.inventory.quantity} onChange={handleChange} className="input input-bordered" min="0" />
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="space-y-6 lg:sticky lg:top-24">
          ...
          <div className="card bg-white shadow-sm border border-gray-200">
            <div className="card-body p-6 space-y-4">
              <h2 className="card-title text-lg font-semibold mb-2">Trạng thái</h2>
          ...
          <div className="card bg-white shadow-sm border border-gray-200">
            <div className="card-body p-6 space-y-4">
              <h2 className="card-title text-lg font-semibold mb-2">Danh mục</h2>
              <div className="space-y-3 max-h-72 overflow-y-auto pr-2">
                {categories.map(cat => (
                  <div key={cat.id} className="flex items-center">
                    ...
                    />
                    <label htmlFor={`category-${cat.id}`} className="ml-3 text-sm font-medium">{cat.name}</label>
                  </div>
                ))}
              </div>
            </div>
